...
function check_explore(ammoBody) {
  ...
  if((x-xx)*(x-xx) + (y-yy)*(y-yy) + (z-zz)*(z-zz) <= 125){
  ...
  for(var i=0; i<51; i++){
    if(ammos[i] && ammos_explore[i] == false){
      console.log(i)
      var xxx = ammos[i].position.x
      var yyy = ammos[i].position.y
      var zzz = ammos[i].position.z
      if((xxx-xx)*(xxx-xx) + (yyy-yy)*(yyy-yy) + (zzz-zz)*(zzz-zz) <= 125){
        ammos_explore[i] = true
        check_explore(ammos[i])
        explore(ammos[i], ammoMeshes[i], 'ammo')
      }
    }
  }
  for(var i=0; i<50; i++){
    ...
    if((xxx-xx)*(xxx-xx) + (yyy-yy)*(yyy-yy) + (zzz-zz)*(zzz-zz) <= 125){
      explore(boxes[i], boxMeshes[i], 'box')
    }
  }
}
  ...
  if (controls.enabled == true) {
    ...
    if (e.which === 1) {
      ...
      if (ammos.length > 50) {
        ...
        ammos.length = 0
        ammoMeshes.length = 0
        ammos_explore.length = 0;
      }
      // 子彈剛體與網格
      const ammoBody = new CANNON.Body({ mass: 200000 })
      // const ammoShape = new CANNON.Sphere(0.0002)
      ammoBody.addShape(ballShape)

      const ammoMaterial = new THREE.MeshStandardMaterial({ color: 0xc3e9ff })
